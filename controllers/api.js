...
exports.getAnalytics = function(req, res) {
  var token = _.find(req.user.tokens, { kind: 'analytics' });
  console.log('token=' + JSON.stringify(token, null, 4));
  var query = querystring.stringify(token.accessToken);
  console.log('query='+query);
  async.parallel({
    getAccountSummaries: function(done) {
      var options = {
      ...
      request.get(options, function(err, request, body) {
        ...
        // console.log('Request_Body=' + JSON.stringify(body, null, 4));
        // console.log('Request_request=' + JSON.stringify(request, null, 4));
        if (request.statusCode !== 200) res.redirect('/api/unlink/analytics');
        done(err, body);
      });
    },
    getAnalyticsMetaData: function(done) {
      var options = {
          //url: 'https://www.googleapis.com/analytics/v3/metadata/ga/columns?fields=items(attributes/type,attributes/status,attributes/group,id)',
          url: 'https://www.googleapis.com/analytics/v3/metadata/ga/columns',
          json: true
      };

      request(options, function (err, resp, body) {
          var meta = {metrics:{}, dimensions:{}, mgroups:[], dgroups:[]};
          var items = body.items;

          for (i = 0; i < items.length; i++){
            if (items[i].attributes.status != 'DEPRECATED'){
              
              var group = items[i].attributes.group;
              var id = items[i].id;
              var type = items[i].attributes.type;

              if (type == 'METRIC'){
                
                if (!meta.metrics[group]){
                  meta.mgroups.push(group);
                  meta.metrics[group] = [];
                  meta.metrics[group].push(id);
                } else {
                  meta.metrics[group].push(id);
                }
                ...
                if (!meta.dimensions[group]){
                  meta.dgroups.push(group);
                  meta.dimensions[group] = [];
                  meta.dimensions[group].push(id);
                } else {
                  meta.dimensions[group].push(id);
                }
  ...
  function(err, results) {
    console.log('Request Next Error=' + err);
    if (err) return next(err);
    //console.log('Result getAccountSummaries=' + JSON.stringify(results.getAccountSummaries, null, 4));
    res.render('api/analytics', {
      title: 'Analytics API',
      accountSummaries: results.getAccountSummaries,
      analyticsMetaData: results.getAnalyticsMetaData
    });
  });
};
...
// format GET https://www.googleapis.com/analytics/v3/data/ga?ids=ga:89603425&start-date=2013-01-01&end-date=2014-11-01&metrics=ga:sessions, ga:users&dimensions=ga:date, ga:city&max-results=100&key={YOUR_API_KEY}
exports.postAnalytics = function(req, res, next) {
  req.assert('profile', 'Google Analytics property cannot be blank').notEmpty();
  req.assert('metrics', 'Please specify at least one metric.').notEmpty();
  req.assert('enddate', 'Please specify an end date.').notEmpty();
  var errors = req.validationErrors();
  if (errors) {
    req.flash('errors', errors);
  ...
  var token = _.find(req.user.tokens, { kind: 'analytics' });
  var query =
    'ids=' + req.body.profile +
    '&start-date=' + req.body.startdate +
    '&end-date=' + req.body.enddate +
    '&metrics=' + req.body.metrics +
    '&dimensions=' + ((req.body.dimensions)? req.body.dimensions : '');
  var analyticsMetaData = JSON.parse(req.body.analyticsMetaData); //te lang voor string waarschijnlijk
  var accountSummaries = JSON.parse(req.body.accountSummaries);
  var postVar = req.body;
  //console.log('accountSummaries='+JSON.stringify(accountSummaries, null, 4));
  console.log('query=' + query);  
  var qs = querystring.escape(query);
  console.log('qs=' + qs);  

  var options = {
      url: 'https://www.googleapis.com/analytics/v3/data/ga?' + query,
      headers: {
          'Authorization': 'Bearer '+ token.accessToken
      },
      json: true
  ...
  async.parallel({
    getData: function(done) {
      ...
      request.get(options, function(err, request, body) {
        ...
        if (request.statusCode !== 200) {
          req.flash('errors', { msg: body.error.message });
          return res.redirect('/api/analytics');
        }
  ...
  function(err, results) {
    req.flash('success', { msg: 'Your report is ready!' });
    console.log(JSON.stringify(results.getData, null, 4));
    res.render('api/analytics', {
      title: 'Analytics API',
